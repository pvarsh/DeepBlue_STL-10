...
\label{train}
Both models were trained with mini batch stochastic gradient descent with batch sizes 1, 8, 32, and 128. Additionally a shorter version of model A1 (two 200-filter convolutional layers, 400-node fully connected layer) was evaluated. The full A1 architecture yielded the best performance on the validation set with mini batch size 8. It was then retrained on the full 5000-image training set and evaluated on the test set.
\begin{center}
...
was trained with stochastic gradient descent using minibatch sizes of 1 (fully stochastic, 8, 32, and 128). The batch size of 8 yielded the smallest validation error.
